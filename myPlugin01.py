...
class MyPlugin:
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/myPlugin01/icon.png'
        self.add_action(
            icon_path,
    ...
    def run(self):
        ...
        if result:
            ...
            self.dlg.get_attr()
            city = self.dlg.city
            category = self.dlg.category
            key = self.dlg.key
            poilist = self.dlg.getpois(city, category, key)
            shpname = self.dlg.filename
            self.dlg.writeShp(shpname, poilist)
            ...
            if vlayer.isValid():
                ...
                # 在QGIS界面上打印结果
                self.iface.messageBar().pushMessage("成功", "加载图层：" + layerName, level=Qgis.Success, duration=5)
            else:
                print("图层加载失败！")
                # 在QGIS界面上打印结果
                self.iface.messageBar().pushMessage("Error", "加载图层失败", level=Qgis.Critical)
